...
class Mod_menu extends CI_Model {
    ...
    public function getMenu()
    {
        ...
        //     return $this->db->where('is_active', 'Y')->where('grup_id', '1')->get('menu')->result();
        // } else if ($this->session->userdata('id') != 2) {
        //     return $this->db->where('is_active', 'Y')->where('grup_id', '2')->get('menu')->result();
        // } else if ($this->session->userdata('id') != 3) {
        //     return $this->db->where('is_active', 'Y')->where('grup_id', '3')->get('menu')->result();
        // } else{
